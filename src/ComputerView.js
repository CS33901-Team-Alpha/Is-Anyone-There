class ComputerView extends View {
    ...
    draw(){
        ...
        if(this.ComputerIsOpen_) {
            ...
            fill(255,255,255);
            textAlign(LEFT, LEFT);
            text("> Type Command for Computer...ENTER to execute", 135, height/2- this.ScreenHeight_/2 + 50);
            text("> " + this.givenCommand_, 135, height/2- this.ScreenHeight_/2 + 115);
            if(this.EnterPressed_) {
                text("> Command : " + this.givenCommand_ + " : EXECUTED", 135, height/2- this.ScreenHeight_/2 + 180);
                text("> Closing Terminal...", 135, height/2- this.ScreenHeight_/2 + 245);
                setTimeout(() => {
                    this.closeComputer();
                }, "2000");
            }
        ...
        if(this.PinIsOpen_) {
            ...
            fill('black');
            textSize(30);
            text(this.PinMessage_, width/2 - this.PinWidth_/2 + 250, height/2- this.PinHeight_/2+50);
            fill('limegreen');
            textSize(50);
            text(this.PinString_ + this.PinAttempt_ , width/2 - this.PinWidth_/2 + 185, height/2- this.PinHeight_/2 +120);
            for (let i = 1; i < 10; i += 1) {
                textSize(100);
                text(i , width/2 - this.PinWidth_/2 +(150*((i-1)%3)) +160, height/2- this.PinHeight_/2 +115+(125*ceil(i/3)));
            }
